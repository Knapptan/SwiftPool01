...
class Zone {
    ...
    func isIncidentInside(incident: Incident) -> Bool {
        ...
        case let .quadrilateral(points):
            ...
            let pointC = points[2]
            let pointD = points[3]
            // Предположим, что у вас есть метод, который проверяет, находится ли точка внутри четырехугольника
...
if shapeArea == "circle" {
    ...
    zone = Zone(phoneNumber: phoneNumber, name: name, emergencyDeptCode: emergencyDeptCode, dangerLevel: dangerLevel, shape: .circle(center: areaParameters.0, radius: areaParameters.1))
    ...
    zone = Zone(phoneNumber: phoneNumber, name: name, emergencyDeptCode: emergencyDeptCode, dangerLevel: dangerLevel, shape: .triangle(points: areaParameters))
    ...
    zone = Zone(phoneNumber: phoneNumber, name: name, emergencyDeptCode: emergencyDeptCode, dangerLevel: dangerLevel, shape: .quadrilateral(points: areaParameters))
}
...
if let incidentTypeString = readLine(), let incidentType = IncidentType(rawValue: incidentTypeString.lowercased()) {
    incident = Incident(description: incidentDescription, applicantNumber: incidentPhoneNumber, incidentType: incidentType, coordinates: incidentCoordinates!)
} else {
